...
def create_mesh(datos):

    gmsh.initialize()


    gmsh.option.setNumber("General.Terminal", 1)
    gmsh.model.add("malla_py")

    #tm = 0.1  # Tamaño de malla a utilizar en todos los puntos
    h = datos.h
    xmin = datos.xmin
    xmax = datos.xmax
    ymin = datos.ymin
    ymax = datos.ymax
    gmsh.model.geo.addPoint(xmin, ymin, 0, h, 1)  
    gmsh.model.geo.addPoint(xmax, ymin, 0, h, 2)  
    gmsh.model.geo.addPoint(xmax, ymax, 0, h, 3)
    gmsh.model.geo.addPoint(xmin, ymax, 0, h, 4)
...
def dirichlet(nodes,datos):
    """
    Busca los nodos que están en el borde
    """
    n = len(nodes[:,0])
    boundary = np.array([])
    for i in range(0,n):
        v = nodes[i,:]
        if v[0]==datos.xmin or v[0]==datos.xmax:
            boundary = np.append(boundary,int(i))
        elif v[1]==datos.ymin or v[1]==datos.ymax:
            boundary = np.append(boundary,int(i))
    return boundary
...
def plot_msh(file, tipo, mostrar_nodos=False, mostrar_num_nodo=False,
    ...
    if tipo=='shell':
        ...
        fig.suptitle(f'Malla de EF 2D', fontsize='x-large')
        ax.set_xlabel('x')
        ax.set_ylabel('y')
